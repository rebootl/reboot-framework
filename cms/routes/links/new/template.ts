...
export type NewLinkFormData = {
  title: string;
  url: string;
  comment: string;
};

const defaultFormData: NewLinkFormData = {
  title: "",
  url: "",
  comment: "",
};

export default (link: NewLinkFormData = defaultFormData) =>
  html`
    <section class="max-w-3xl mx-auto space-y-6">
      <form
        class="space-y-6 rounded-xl bg-dark-surface/50 border border-dark-border p-8 pt-6"
        action="/cms/links/new"
        method="POST"
      >
        <header class="space-y-2">
          <h1 class="text-2xl font-bold text-white">Add Link</h1>
        </header>
        <div class="space-y-4">
